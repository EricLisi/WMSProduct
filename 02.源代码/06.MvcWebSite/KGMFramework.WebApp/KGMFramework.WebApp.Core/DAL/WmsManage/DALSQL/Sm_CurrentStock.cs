...
namespace KGMFramework.WebApp.DALSQL
{
    ...
    /// 库存现存量表
    /// </summary>
	public class Sm_CurrentStock : BaseDALSQL<Sm_CurrentStockInfo>, ISm_CurrentStock
	{
		#region 对象实例及构造函数

		public static Sm_CurrentStock Instance
		{
			get
			{
				return new Sm_CurrentStock();
			}
		}
		public Sm_CurrentStock() : base("Sm_CurrentStock","F_Id")
		{
		}
		...
		protected override Sm_CurrentStockInfo DataReaderToEntity(IDataReader dataReader)
		{
			Sm_CurrentStockInfo info = new Sm_CurrentStockInfo();
			SmartDataReader reader = new SmartDataReader(dataReader);
			
			info.F_Id = reader.GetString("F_Id");
			info.F_Barcode = reader.GetString("F_Barcode");
			...
			info.FSn = reader.GetString("F_SN");
			info.F_Qty = reader.GetDecimal("F_Qty");
			info.F_Box = reader.GetString("F_Box");
        ...
        protected override Hashtable GetHashByEntity(Sm_CurrentStockInfo obj)
		{
		    Sm_CurrentStockInfo info = obj as Sm_CurrentStockInfo;
			Hashtable hash = new Hashtable(); 
			
			hash.Add("F_Id", info.F_Id);
 			hash.Add("F_Barcode", info.F_Barcode);
 			...
 			hash.Add("F_SN", info.FSn);
 			hash.Add("F_Qty", info.F_Qty);
 			hash.Add("F_Box", info.F_Box);
