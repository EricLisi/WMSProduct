...
$.modalOpen = function (options) {
    ...
    options.width = top.$(window).width() > parseInt(options.width.replace('px', '')) ? options.width : top.$(window).width() + 'px';
    options.height = (top.$(window).height() > parseInt(options.height.replace('px', '')) + 50 ? parseInt(options.height.replace('px', '')) + 50 : top.$(window).height()) + 'px';
    var _this = top.layer
    _this.open({
        id: options.id,
        type: 2,
...
$.fn.authorizeButton = function () {
    if (top.$(".NFine_iframe:visible").attr("id")) {
        ...
        var $element = $(this);
        var anthorizeBtns = $element.find('a[authorize=yes]');
        anthorizeBtns.attr('authorize', 'no')
        for (var i = 0; i < anthorizeBtns.length; i++) {
            var btn = $(anthorizeBtns[i]); 
            if (dataJson != undefined) {
                for (var j = 0; j < dataJson.length; j++) { 
                    if (dataJson[j].F_EnCode == btn.attr('id')) {
                        btn.attr('authorize', 'yes');
                    }
                }
            }
        } 
...
$.fn.DefalutMonthFirst = function () {//默认值设置 默认为时间格式
    var _this = $(this);
    var myDate = new Date();
    var month = myDate.getMonth() + 1;
    var day = myDate.getDate();
    var value = myDate.getFullYear() + "-" + (month < 10 ? '0' + month : month) + "-01";
