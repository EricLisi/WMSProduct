...
namespace KGMFramework.WebApp.Areas.WmsManage.Controllers
{
    ...
    /// 入库通知单控制器
    /// </summary>
    public class InStockNoticeController : BusinessController<InStockNotice_Head, InStockNotice_HeadInfo>
    {
        /// <summary>
        /// 入库通知单查询
        /// </summary>
        ...
        public ActionResult GetListGridJson(string keyValue, string sidx = " F_EnCode ", string sord = "desc")
        {
            ...
            List<InStockNotice_BodyInfo> bodylist = BLLFactory<InStockNotice_Body>.Instance.Find(GetConditionStr(condition));
        ...
        /// <returns></returns>
        public ActionResult SubmitForm1(InStockNotice_HeadInfo info, List<InStockNotice_BodyInfo> Binfo)
        {
            try
            {
                ...
                var result = BLLFactory<InStockNotice_Head>.Instance.Save(info, Binfo);
        ...
        public override ActionResult DeleteForm(string keyValue, bool bLogicDelete = false)
        {
            SearchCondition condition = new SearchCondition();
            condition.AddCondition("F_HeadId", keyValue, SqlOperator.Equal);
            BLLFactory<InStockNotice_Body>.Instance.DeleteByCondition(condition.BuildConditionSql().Replace("Where (1=1)  AND ", string.Empty));
            BLLFactory<InStockNotice_Head>.Instance.Delete(keyValue);
            return Success("删除成功");
        }
        ...
        public ActionResult Verify(string KeyValue)
        {
            try
            {
                InStockNotice_HeadInfo info = BLLFactory<InStockNotice_Head>.Instance.FindByID(KeyValue);
                if (info.F_Status == 1)
                {
                    return Error("单据已审核");
                }
                ...
                BLLFactory<InStockNotice_Head>.Instance.Update(info, info.F_Id);
        ...
        protected override ResultModel DoAfterUploadFile(string fullFileName)
        {
            try
            {
                ...
                var res = BLLFactory<InStockNotice_Head>.Instance.UploadExcel(dt, CurrentUser.F_Id);
