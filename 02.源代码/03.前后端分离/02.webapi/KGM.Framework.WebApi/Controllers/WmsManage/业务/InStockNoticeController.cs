...
namespace KGM.Framework.WebApi.Controllers
{
    ...
    //[ApiController]
    public class InStockNoticeController : ControllerBase
    {
        #region 依赖注入
        IInStockNoticeHeadService _service;
        IInStockNoticeBodyService _inStockNoticeBodyService;
        ...
        /// <param name="inStockNoticeBodyService"></param>
        /// <param name="customerService"></param>
        /// <param name="warehouseService"></param>
        /// <param name="productService"></param>
        /// <param name="ownerService"></param>
        /// <param name="strtypeService"></param>
        public InStockNoticeController(IInStockNoticeHeadService service, IInStockNoticeBodyService inStockNoticeBodyService, ICustomerService customerService, IWarehouseService warehouseService, IInventoryService productService, IOwnerService ownerService, IReceiveTypeService strtypeService)
        {
            this._warehouseService = warehouseService;
            this._customerService = customerService;
            this._service = service;
            this._inStockNoticeBodyService = inStockNoticeBodyService;
        ...
        /// <returns></returns>
        [HttpPost, Route("")]
        public async Task<IActionResult> Add([FromBody]GetInStockNoticeModel model)
        {
            return await Task.Run(() =>
            {
                var Message = _service.InsUpdateInStockNotice(GetTableData(model));
        ...
        /// <returns></returns>
        [HttpPost, Route("Fastshelf")]
        public async Task<IActionResult> Fastshelf([FromBody]QuickShelfModel model)
        {
            return await Task.Run(() =>
            {
                var Message = _service.QuickShelf(model.orderNo, model.PositionCode, model.User);
                ReturnModel @return = Message as ReturnModel;
        ...
        /// <returns></returns>
        [HttpPut, Route("{Id}")]
        public async Task<IActionResult> Update([FromBody]GetInStockNoticeModel model, string Id)
        {
            return await Task.Run(() =>
            {
                model.info.Id = Id;
        ...
        public async Task<IActionResult> Delete(string Id)
        {
            return await Task.Run(() =>
            {
                ReturnModel model = _service.DeleteInStockNotice(Id);
        ...
        /// <returns></returns>
        [HttpGet, Route("QuickPU/{orderNo}/{user}")]
        public async Task<IActionResult> QuickPU(string orderNo,string user)
        {
            ...
            return await Task.Run(() =>
            {
                ...
                foreach (string str in orderNo.Split(','))
                {
                ...
                var Message = _service.QuickPU(dt,user);
                ReturnModel @return = Message as ReturnModel;

                if (@return.r == 1 )
                {
                    return Ok(new { Status = true, Message = "操作成功" });
        ...
        public async Task<IActionResult> GetById(string Id)
        {
            ...
            var Dinfo = await _inStockNoticeBodyService.QueryAsync(u => u.OrderNo == Info.OrderNo);
        ...
        public async Task<IActionResult> GatPagerListByWhere(string keyword, int rows, int page, string sidx, string sord, string Id)
        {
            ...
            PagerEntity<InStockNoticeHeadGetDto> entity = await _service.QueryByPagesAsync(rows, page,
                      t => t.OrderNo.Contains((keyword ?? t.OrderNo)) ,
                      _service.Expression(sidx), sord.ToUpper().Equals("ASC"));

            List<InStockNoticeHeadGetDto> list = await _service.QueryAsync();
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        public static DataSet GetTableData(GetInStockNoticeModel model)
        {
            DataSet ds = GetData();
