(function ($, app) {
    "use strict";
    var $appgridmove = null;
    var cb = ['checkbox_0.png', 'checkbox_1.png', 'checkbox_2.png'];
    var imageurl = '../../../../assets/images/grid/';
    ...
    var setLastColWidth = function ($self, op, isUpdate) {
        ...
        if (op.running.cols.length > 0) {
            ...
            if (flag) {
                $self.find('#appgrid_head_col_' + op.id).css({ 'width': (op.running.headWidth + _width) });
                if (isUpdate) {
                    $self.find('#appgrid_body_' + op.id + '>.app-scroll-box').css({ 'width': (op.running.headWidth + op.running.leftWidth + _width - 1) });
                    $self.find('#appgrid_right_' + op.id).css({ 'width': (op.running.headWidth + _width - 1) });
                    $self.find('#appgrid_right_' + op.id + '>[colname="' + col.data.name + '"]').css({ 'width': col._width });
                }
            }
    ...
    var formatterCell = function (value, item, row, rowItem, op) {
        ...
        if (item.formatter) {
            ...
            var $expend = rowItem.$cell.find('.appgrid-data-cell-expend');
            ...
            if (item.statistics) {// 如果该单元格数据需要统计就进行计算
                ...
                $('#appgrid_statistic_' + op.id + ' [name="' + item.name + '"]').text(op.running.statisticData[item.name]);
            }
                ...
                switch (item.edit.type) {
                            ...
                            if (item.edit.datatype === 'dataItem') {
                                app.clientdata.getAsync('dataItem', {
                                    ...
                                    callback: function (_data,_op) {
                                        ...
                                        var $expend = _op.rowItem.$cell.find('.appgrid-data-cell-expend');
                                ...
                                app.clientdata.getAsync('sourceData', {
                                    key: value,
                    ...
                    case 'datatime':       // 时间
                        if (item.edit.dateformat == "0") {                       
                           // value =  app.BASE_UTILS.Date.get_yyyy_MM_dd(value);
                            row[item.name] = value;
                        }
                    ...
                    case 'layer':          // 弹层
                        ...
                        rowItem.$cell.find('.fa-ellipsis-h').on('click', function () {
                            ...
                            top.appGirdLayerEdit = op;
                            top.appGirdLayerEditCallBack = function (data) {
                                op.edit.change && op.edit.change(_row, rownum, data);
                                top.appGirdLayerEdit = null;
                                top.appGirdLayerEditCallBack = null;
                            };
                            if (item.edit.op) {
                                /*app.layerForm({
                                    id: 'appgridlayerform',
                               ...
                               //alert($("#EnabledMark").prop("checked"))
                               //var dd=$("#EnabledMark").prop("checked");
                                                            
                                layx.iframe('appgridlayerform','选择' + item.label,'../../../Utility/JfGirdLayerForm/html/JfGirdLayerForm.html',{	
                                	shadable: true,
                                	statusBar:true,
                                	height: item.edit.op.height || 400,
                                    width: item.edit.op.width || 601,
								    buttons:[
								        {
								            ...
								            callback:function(id, button, event){								            	
								                ...
								                var res =  contentWindow.acceptClick(function (data) {
			                                        op.edit.change && op.edit.change(_row, rownum, data);
			                                   });
			                                    layx.destroy(id);
			                                    top.appGirdLayerEdit = null;
...
//								                	var bb=$('.aa').appGridGet('rowdatas');
//								                	var c = data.concat(bb);				
...
//			                                    return res;			                                    
    ...
    var initEditCell = function ($self, op, col) {
        switch (col.data.edit.type) {
            case 'input':          // 输入框 文本,数字,密码
                col.$edit = $('<div class="appgrid-edit-cell"><input id="appgrid_edit_' + op.id + '_' + col.data.name + '" /></div>');
                if (col.frozen) {
                    $self.find('#appgrid_left_' + op.id).append(col.$edit);
                }
                else {
                    $self.find('#appgrid_right_' + op.id).append(col.$edit);
                }
                ...
                break;
            case 'select':         // 下拉框选择
                col.$edit = $('<div class="appgrid-edit-cell"><div id="appgrid_edit_' + op.id + '_' + col.data.name + '" ></div></div>');
                if (col.frozen) {
                    $self.find('#appgrid_left_' + op.id).append(col.$edit);
                }
                else {
                ...
                col.$edit.find('div').appselect(col.data.edit.op);

                col.data.edit.op.value = col.$edit.find('div')[0]._appselect.dfop.value;
                col.data.edit.op.text = col.$edit.find('div')[0]._appselect.dfop.text;
                if (col.data.edit.datatype == 'dataItem') {
                    app.clientdata.getAllAsync('dataItem', {
                        ...
                        callback: function (dataes) {
                            ...
                            col.$edit.find('div').appselectRefresh({ data: list });
                        }
                    });
                }
                    ...
                    app.clientdata.getAllAsync('sourceData', {
                        code: col.data.edit.code,
                        callback: function (dataes) {
                            col.$edit.find('div').appselectRefresh({
                                data: dataes
                            });
                        }
                    });
                ...
                break;
            case 'radio':          // 单选
                col.$edit = $('<div class="appgrid-edit-cell"><div id="appgrid_edit_' + op.id + '_' + col.data.name + '" class="radio"  ></div></div>');
                if (col.frozen) {
                    $self.find('#appgrid_left_' + op.id).append(col.$edit);
                }
                else {
                ...
                if (col.data.edit.datatype == 'dataItem') {
                    col.$edit.find('div').appRadioCheckbox({
                        type: 'radio',
                        code: col.data.edit.code
                    });
                }
                    ...
                    $.each(col.data.edit.data || [], function (id, item) {
                        var $point = $('<label><input name="appgrid_edit_' + op.id + '_' + col.data.name + '" value="' + item.id + '"' + (col.data.edit.dfvalue == item.id ? "checked" : "") + ' type="radio">' + item.text + '</label>');
                        col.$edit.find('div').append($point);
                    });
                }
                break;
            case 'checkbox':       // 多选
                col.$edit = $('<div class="appgrid-edit-cell"><div id="appgrid_edit_' + op.id + '_' + col.data.name + '" class="checkbox"  ></div></div>');
                if (col.frozen) {
                    $self.find('#appgrid_left_' + op.id).append(col.$edit);
                }
                else {
                ...
                if (col.data.edit.datatype == 'dataItem') {
                    ...
                    $.each(col.data.edit.data || [], function (id, item) {
                        var $point = $('<label><input name="appgrid_edit_' + op.id + '_' + col.data.name + '" value="' + item.id + '"' + (col.data.edit.dfvalue == item.id ? "checked" : "") + ' type="checkbox">' + item.text + '</label>');
                        col.$edit.find('div').append($point);
                    });
                }
                break;
            case 'datatime':       // 时间
                ...
                col.$edit = $('<div class="appgrid-edit-cell"><input id="appgrid_edit_' + op.id + '_' + col.data.name + '" onClick="WdatePicker({dateFmt:\'' + dateformat + '\',qsEnabled:false,isShowClear:false,isShowOK:false,isShowToday:false,onpicked:function(){$(\'#appgrid_edit_' + op.id + '_' + col.data.name + '\').trigger(\'change\');}})"  type="text" class="form-control" /></div>');
                if (col.frozen) {
                    $self.find('#appgrid_left_' + op.id).append(col.$edit);
                }
                else {
    ...
    var hideEditCell = function () {
        $('.appgrid-layout .appgrid-edit-cell ').hide();
        $('.appgrid-layout .app-select-option').slideUp(150);
        $('.appgrid-layout .app-select').removeClass('app-select-focus');
    };
    ...
    var _appgrid = {
        init: function ($self, op) {
            ...
            $self.html('');
            $self.addClass('appgrid-layout');
            // 添加一个移动板
            if ($appgridmove === null) {
                $appgridmove = $('<div style="position: fixed;top: 0;left: 0;width: 100%;height: 100%;z-index: 9999;cursor: col-resize;display: none;" ></div>');
                $('body').append($appgridmove);
            }
            // 整体表格布局
            _appgrid.layout($self, op);
            _appgrid.bind($self, op);
            // 渲染头部
            _appgrid.head($self, op);
            // 渲染数据
            _appgrid.dataRender($self, op, op.rowdatas);
            op = null;
        },
        layout: function ($self, op) {
            ...
            // 头部
            var $head = $('<div class="appgrid-head" id="appgrid_head_' + op.id + '" ></div>');
            $head.append('<div class="appgrid-border" id="appgrid_border_' + op.id + '" ></div>');
            $head.append('<div class="appgrid-head-col" id="appgrid_head_col_' + op.id + '" ></div>');
            $self.append($head);
            // 数据显示部分
            var $body = $('<div class="appgrid-body" id="appgrid_body_' + op.id + '" ></div>');
            $body.append('<div class="appgrid-left" id="appgrid_left_' + op.id + '" ></div>');
            $body.append('<div class="appgrid-right" id="appgrid_right_' + op.id + '" ></div>');
            $self.append($body);
            // 底部
            var $footer = $('<div class="appgrid-footer" id="appgrid_footer_' + op.id + '" ></div>');
            $self.append($footer);

            // 调整列宽移动条
            $self.append('<div class="appgrid-move-line" id="appgrid_move_line_' + op.id + '"  ></div>');

        
            // 初始化滚动条
            $body.appscroll(function (x, y) {
                if (!$self.is(":hidden")) {
                    $self.find('#appgrid_left_' + op.id).css('left', x);
                    $self.find('#appgrid_head_col_' + op.id).css('left', op.running.leftWidth - x);
                    if (op.running.isStatistic) {// 添加统计条单元格
                        $self.find('#appgrid_statistic_' + op.id + '>.appgrid-statistic-right').css('left', - x);
                    }
            ...
            // 没有数据的时候显示的图片
            $self.find('#appgrid_body_' + op.id).append('<div class="appgrid-nodata-img" id="appgrid_nodata_img_' + op.id + '"  ><img src="../../../../assets/images/grid/nodata.jpg"></div>');

            // 分页条
            if (op.isPage) {// 支持
                $self.css({ 'padding-bottom': '35px' });
                var $pagebar = $('<div class="appgrid-page-bar" id="appgrid_page_bar_' + op.id + '"><div class="appgrid-page-bar-info" >无显示数据</div>\
                <div class="paginations" id="appgrid_page_bar_nums_'+ op.id + '" style="display:none;" >\
                ...
                <ul class="pagination pagination-sm" id="appgrid_page_bar_num_' + op.id + '" ></ul>\
                ...
                $pagebar.find('#appgrid_page_bar_num_' + op.id).on('click', _appgrid.turnPage);
                $pagebar.find('#appgrid_page_bar_nums_' + op.id + ' .pagebtn').on('click', { op: op }, _appgrid.turnPage2);
                $pagebar = null;
            }
            else if (op.isEdit) {
                $self.css({ 'padding-bottom': '29px' });
                var $toolbar = $('<div class="appgrid-toolbar" id="appgrid_toolbar_' + op.id + '"></div>');
                var $add = $('<span><i class="fa fa-plus"></i></span>');
                var $minus = $('<span><i class="fa fa-minus"></i></span>');
                var $fuzhi = $('<span class=""><i class="fa fa-file-audio-o"></i></span>');
                ...
                $add.on('click', function () {
                    ...
                    if (op.isTree) {
                        _appgrid.rowRender($self, op, { data: _item, childRows: [] }, 0, 1);
                    }
                    else {
                        _appgrid.rowRender($self, op, _item, 0);
                    }
                    $self.find('#appgrid_nodata_img_' + op.id).hide();
                ...
                $minus.on('click', function () {
                    ...
                    if (op.isMultiselect) {
                        ...
                        $.each(op.running.rowdata, function (_index, _item) {
                            if (_item['jfcheck'].value == 1) {
                                ...
                                if (op.beforeMinusRow) {
                                    res = op.beforeMinusRow(_item['appgridRowData']);
                                }
                                if (res) {
                                    op.rowdatas.splice(op.rowdatas.indexOf(_item['appgridRowData']), 1);
                                    checklist.push(_item['appgridRowData']);
                                    flag = true;
                                }
                            }
                        });
                        ...
                        if (op.running.rowSelected != null) {
                            res = true;
                            if (op.beforeMinusRow) {
                                res = op.beforeMinusRow(op.running.rowSelected['appgridRowData']);
                            }
                            if (res) {
                    ...
                    if (op.running.rowdata.length == 0) {
                        $self.find('#appgrid_nodata_img_' + op.id).show();
                    }
                });
//              //复制行
...
//                              if (res) {
//                              	alert(44444444444)
//                              	alert(JSON.stringify(op.rowdatas))
//		                            op.rowdatas.push(_item);
//		                            op.onAddRow && op.onAddRow(_item, op.rowdatas);
//                                  flag = true;
//                              }
...
//              });
        ...
        bind: function ($self, op) {
            ...
            $self.on('click', function (e) {
                ...
                if (!$et.hasClass('appgrid-edit-cell') && $et.parents('.appgrid-edit-cell').length == 0) {
                    hideEditCell();
                }

                if (op.running.isWidhChanging) {// 调整列表宽度
                    //top.appgrid.moveHeadWidth(dfop, false);
                    dfop.isWidhChanging = false;
                }
                else if ($et.hasClass('appgrid-head-cell') || $et.parents('.appgrid-head-cell').length > 0) {// 排序
                    if (!$et.hasClass('appgrid-head-cell')) {
                        $et = $et.parents('.appgrid-head-cell');
                    }
                    _appgrid.sortCol($this, $et, op);
                }
                else if ($et.attr('colname') == 'appgrid_subGrid' || $et.parent().attr('colname') == 'appgrid_subGrid') {// 展开或关闭子表单
                    _appgrid.expandSub($this, $et, op);
                }
                else if ($et.parent().hasClass('appgrid-data-cell-expend')) {// 树形结构展开和关闭
                    _appgrid.expandTree($et, op);
                }
                else if ($et.hasClass('appgrid-data-cell') || $et.parents('.appgrid-data-cell').length > 0) {// 选中行
                    _appgrid.clickRow($this, $et, op);
                    e.stopPropagation();
                }
                else if ($et.attr('id') == ('appgrid_all_cb_' + op.id)) {// 全部勾选
                    _appgrid.checkAlappows($this, $et, op);
                }
            });
            // 双击事件
            $self.on('dblclick', function (e) {
                ...
                var et = e.target || e.srcElement;
                var $et = $(et);
                if ($et.hasClass('appgrid-data-cell') || $et.parents('.appgrid-data-cell').length > 0) {// 选中行
                    op.dblclick && op.dblclick(op.running.rowSelected['appgridRowData']);
                }
            });
            ...
            $self.on('mouseover', function (e) {
                var $this = $(this);
                $this.find('.appgrid-data-cell-over').removeClass('appgrid-data-cell-over');

                var et = e.target || e.srcElement;
                var $et = $(et);
                if ($et.hasClass('appgrid-data-cell') || $et.parents('.appgrid-data-cell').length > 0) {
                    var rowid = $et.attr('rownum');
                    if (!rowid) {
                        rowid = $et.parents('.appgrid-data-cell').attr('rownum');
                    }
                    $this.find('[rownum="' + rowid + '"]').addClass('appgrid-data-cell-over');
                }
            ...
            $self.resize(function () {
                ...
                // 如果有子表格调整宽度
                var width = $this.find('#appgrid_body_' + op.id).width();
                $this.find('.appgrid-sub').css({ 'width': width });
            });
            // 调整列宽
            $self.delegate('.appgrid-heed-move', 'mousedown', { op: op }, function (e) {
                $appgridmove.show();
                var op = e.data.op;
                op.running.moveing = true;
                op.running.xMousedown = e.pageX;
                var $moveline = $('#appgrid_move_line_' + op.id);
            ...
            top.$(document).on('mousemove', { $obj: $self }, function (e) {
                ...
                if (op.running.moveing) {
                    var $moveline = e.data.$obj.find('#appgrid_move_line_' + op.id);
                    ...
                    if (op.running.moveCol) {
                        ...
                        if (_width != 0){
                            ...
                            if (op.running.moveCol.frozen) {
                                ...
                                e.data.$obj.find('#appgrid_head_' + op.id).css({ 'padding-left': op.running.leftWidth });
                                e.data.$obj.find('#appgrid_border_' + op.id).css({ 'width': op.running.leftWidth });
                                e.data.$obj.find('#appgrid_head_col_' + op.id).css({ 'left': op.running.leftWidth });
                                e.data.$obj.find('#appgrid_body_' + op.id + '>.app-scroll-box').css({ 'padding-left': op.running.leftWidth });
                                e.data.$obj.find('#appgrid_left_' + op.id).css({ 'width': op.running.leftWidth });
                                e.data.$obj.find('#appgrid_left_' + op.id + '>[colname="' + op.running.moveCol.data.name + '"]').css({ 'width': op.running.moveCol.width });

                                if (op.running.isStatistic) {
                                    e.data.$obj.find('#appgrid_statistic_' + op.id).css({ 'padding-left': op.running.leftWidth });
                                    e.data.$obj.find('#appgrid_statistic_' + op.id + '>.appgrid-statistic-left').css({ 'width': op.running.leftWidth });
                                    e.data.$obj.find('#appgrid_statistic_' + op.id + ' [name="' + op.running.moveCol.data.name + '"]').css({ 'width': op.running.moveCol.width });
                                }
                            }
                            else {
                                ...
                                e.data.$obj.find('#appgrid_head_col_' + op.id).css({ 'width': op.running.headWidth });
                                e.data.$obj.find('#appgrid_right_' + op.id).css({ 'width': op.running.headWidth });
                                e.data.$obj.find('#appgrid_right_' + op.id + '>[colname="' + op.running.moveCol.data.name + '"]').css({ 'width': op.running.moveCol.width });


                                if (op.running.isStatistic) {
                                    e.data.$obj.find('#appgrid_statistic_' + op.id + '>.appgrid-statistic-right').css({ 'width': op.running.headWidth });
                                    e.data.$obj.find('#appgrid_statistic_' + op.id + ' [name="' + op.running.moveCol.data.name + '"]').css({ 'width': op.running.moveCol.width });
                                }
                            ...
                            for (var i = path + 1, l = op.running.headData.length; i < l; i++) {
                                ...
                                if (col.frozen && op.running.moveCol.frozen) {
                                    ...
                                    e.data.$obj.find('#appgrid_left_' + op.id + '>[colname="' + col.data.name + '"]').css({ 'left': col.left + op.running.leftWidth - op.running.frozenleft});


                                    if (op.running.isStatistic) {
                                        e.data.$obj.find('#appgrid_statistic_' + op.id + ' [name="' + col.data.name + '"]').css({ 'left': col.left + op.running.leftWidth - op.running.frozenleft });
                                    }
                                }
                                else if (!col.frozen && !op.running.moveCol.frozen) {
                                    col.left += _width;
                                    col.$cell.css({ 'left': col.left});
                                    e.data.$obj.find('#appgrid_right_' + op.id + '>[colname="' + col.data.name + '"]').css({ 'left': col.left });
                                    if (op.running.isStatistic) {
                                        e.data.$obj.find('#appgrid_statistic_' + op.id + ' [name="' + col.data.name + '"]').css({ 'left': col.left });
                                    }
                                }
                            }
        ...
        head: function ($self, op) {
            ...
            if (op.running.isStatistic) {
                ...
                // 判断是否加载了底部条
                $self.find('#appgrid_footer_' + op.id).append('<div class="appgrid-statistic" id="appgrid_statistic_' + op.id + '"><div class="appgrid-statistic-left"></div><div class="appgrid-statistic-right" ></div></div>');
                if (op.isPage) {
                    $self.find('#appgrid_footer_' + op.id).css({ 'height': 64, 'padding-top': '29px' });
                    $self.css({ 'padding-bottom': '64px' });
                }
                else {
                    $self.find('#appgrid_footer_' + op.id).css({ 'height': 29 });
                    $self.css({ 'padding-bottom': '29px' });
                }
                $self.find('#appgrid_toolbar_' + op.id).css({'width':'100px'});
            } else if (op.isEdit) {
                $self.find('#appgrid_toolbar_' + op.id).css({ 'width': '100%' });
            }
            
            op.running.headHeight = op.running.MaxDeep * 28 + 28;
            op.running.headWidth = op.running.left;

            var $border = $self.find('#appgrid_border_' + op.id);
            var $headcol = $self.find('#appgrid_head_col_' + op.id);

            // 判断是否有序号列
            if (op.isShowNum) {
                $border.append('<div class="appgrid-border-cell appgrid-border-num"></div>');
                op.running.leftWidth += 30;
            }
            // 判断是否允许多选
            if (op.isMultiselect) {
                var $cb = $('<div class="appgrid-border-cell appgrid-border-cb"><img  id="appgrid_all_cb_' + op.id + '" src="' + imageurl + cb[0] + '" /></div>')
            ...
            // 是否有展开
            if (op.isSubGrid) {
                var $sub = $('<div class="appgrid-border-cell appgrid-border-sub"></div>').css('left', op.running.leftWidth);
                $border.append($sub);
                op.running.leftWidth += 30;
            }
            op.running.leftWidth += op.running.frozenleft;
            $self.css({ 'padding-top': op.running.headHeight });
            $self.find('#appgrid_head_' + op.id).css({ 'padding-top': op.running.headHeight });
            ...
            $self.find('#appgrid_body_' + op.id +'>.app-scroll-box').css({ 'padding-left': op.running.leftWidth });
            $self.find('#appgrid_left_' + op.id).css({ 'width': op.running.leftWidth });

            // 设置最后一列的宽度
            setLastColWidth($self, op, false);

            if (op.running.isStatistic) {// 添加统计条单元格
                $self.find('#appgrid_statistic_' + op.id).css({ 'padding-left': op.running.leftWidth });
                $self.find('#appgrid_statistic_' + op.id + '>.appgrid-statistic-left').css({ 'width': op.running.leftWidth });
            }
            ...
            $.each(op.running.headData, function (_index, _item) {
                ...
                _item.$cell = $('<div class="appgrid-head-cell" path="' + _index + '"  ><span>' + (_item.data.label || "") + '</span></div>')
                ...
                if (_item.last) {
                    _item.$cell.append('<div class="appgrid-heed-sort"><i class="fa fa-caret-up"></i><i class="fa fa-caret-down"></i></div>');
                    _item.$cell.append('<div class="appgrid-heed-move"></div>');
                    ...
                    if (op.running.isStatistic) {// 添加统计条单元格
                        ...
                        if (_item.frozen) {
                            ...
                            $self.find('#appgrid_statistic_' + op.id + '>.appgrid-statistic-left').append($sLast);
                        }
                        else {
                            $self.find('#appgrid_statistic_' + op.id + '>.appgrid-statistic-right').append($sLast);
                        }
            ...
            $self.find('#appgrid_right_' + op.id).parent().css({ 'width': op.running.leftWidth + _left - 1 });
            $self.find('#appgrid_right_' + op.id).css({ 'width': _left - 1 });

            if (op.running.isStatistic) {// 添加统计条单元格\
                $self.find('#appgrid_statistic_' + op.id + '>.appgrid-statistic-right').css({ 'width': _left - 1 });
            }
        },
        ...
        dataRender: function ($self, op, data, bindex) {
            ...
            op.running.rowdata = [];
            op.running.statisticData = {};
            $self.find('#appgrid_left_' + op.id).find('.appgrid-data-cell').remove();
            $self.find('#appgrid_right_' + op.id).find('.appgrid-data-cell').remove();
            hideEditCell();
            var isAllCheck = 3;
            if (op.isTree) {
                var treeList = _appgrid.listTotree(data, op);
                ...
                function _treeRender(_data, deep) {
                    $.each(_data, function (_index, _item) {
                        _appgrid.rowRender($self, op, _item, bindex, deep);
                ...
                $.each(data, function (_index, _item) {
                    _appgrid.rowRender($self, op, _item, bindex);
                ...
                if (op.isPage) { // 如果是分页，刷新一下分页显示条
                    var $pagebar = $('#appgrid_page_bar_' + op.id);
                    var $pagebarBtn = $pagebar.find('#appgrid_page_bar_num_' + op.id);
                    var $pagebarBtns = $pagebar.find('#appgrid_page_bar_nums_' + op.id);
                    ...
                    $pagebar.find('.appgrid-page-bar-info').html(pagebarLabel);

                }
            }
            if (op.isMultiselect) {
                if (isAllCheck == 2 || isAllCheck == 1) {
                    $self.find('#appgrid_all_cb_' + op.id).attr('src', imageurl + cb[isAllCheck]);
                }
                else {
                    $self.find('#appgrid_all_cb_' + op.id).attr('src', imageurl + cb[0]);
                }
            }
        ...
        rowRender: function ($self, op, _item, bindex, deep) {
            ...
            var $left = $self.find('#appgrid_left_' + op.id);
            var $right = $self.find('#appgrid_right_' + op.id);
            ...
            var $expendcell = null;
            if (op.isTree) {
                row['appgridRowData'] = _item.data;
                row.childRows = _item.childRows;
                row.jfdeep = deep;
                _item = _item.data;
                $expendcell = $('<div class="appgrid-data-cell-expend" style="width:' + (deep * 16) + 'px;" ></div>');
                if (row.childRows.length > 0) {
                    $expendcell.append('<i class="fa fa-caret-down appgrid-data-cell-expendi"></i>');
                }
                op.rowdatas[_index] = _item;

            } else {
                row['appgridRowData'] = _item;
            }
            ...
            if (op.isShowNum) {
                row['jfnum'].$cell = $('<div class="appgrid-data-cell" rowindex="' + _index + '" rownum="' + op.id + '_' + _index + '" colname="appgrid_num"  >' + (_index + bindex * (op.isPage ? op.running.pageparam.rows : 1)+ 1) + '</div>');
            ...
            if (op.isMultiselect) {
                ...
                row['jfcheck'] = {
                    ...
                    $cell: $('<div class="appgrid-data-cell" rowindex="' + _index + '" rownum="' + op.id + '_' + _index + '" colname="appgrid_check"  ><img src="' + imageurl + cb[0] + '" /></div>')
                };
            ...
            if (op.isSubGrid) {
                row['jfsubGrid'] = {
                    ...
                    left: _left,
                    value: false,
                    $cell: $('<div class="appgrid-data-cell"  rowindex="' + _index + '"  rownum="' + op.id + '_' + _index + '" colname="appgrid_subGrid"  ><i class="fa fa-plus" ></i></div>')
                };
            ...
            $.each(op.running.frozenCols, function (_id, col) {
                ...
                row[col.data.name] = {
                    ...
                    left: _left,
                    value: value,
                    $cell: $('<div class="appgrid-data-cell" rowindex="' + _index + '" rownum="' + op.id + '_' + _index + '" colname="' + col.data.name + '"  ></div>')
                };
            ...
            if (op.running.rowSelected && lastCell) {
                if (op.running.rowSelected.appgridRowData[op.mainId]) {
                    op.running.rowSelected.appgridRowData[op.mainId] == row.appgridRowData[op.mainId] && (lastCell.$cell.trigger('click'));
                }
                else {
        ...
        updateRow: function (row, op) {
            ...
            $.each(row, function (id, _item) {
                if (id != 'jfnum' && id != 'jfcheck' && id != 'jfsubGrid' && id != 'appgridRowData') {
                    if (_item.value != row['appgridRowData'][id]) {
                        _item.value = row['appgridRowData'][id] || '';
                        formatterCell(_item.value, map[id], row['appgridRowData'], _item, op);
                    }
                }
            });
        },
...
//              app.loading(true, '正在获取数据');
                ...
                $.extend(true, op.param, op.running.pageparam);                 
//              alert('PARM='+JSON.stringify(op.param))
              layx.load('loadId', '数据正在加载中，请稍后');
                app.HTTP_REQUEST_UTILS.httpAsync('GET', op.url, op.param, function (data) {
                	layx.destroy('loadId');
                    if (data) {
                    ...
                    _appgrid.dataRender($self, op, op.rowdatas, (op.running.pageparam.page - 1));
...
//          	layx.load('loadId', '数据正在加载中，请稍后');
                app.HTTP_REQUEST_UTILS.httpAsync('GET', op.url, op.param, function (data) {
                	layx.destroy('loadId');
                    op.rowdatas = data || [];
                    _appgrid.dataRender($self, op, op.rowdatas, 0);
                    op.onRenderComplete && op.onRenderComplete(op.rowdatas);
                },()=>{ 
                    layx.destroy('loadId');
                });
        ...
        expandTree: function ($et, op) {
            ...
            $('#appgrid_left_' + op.id + ' .appgrid-selected,#appgrid_right_' + op.id + ' .appgrid-selected').removeClass('appgrid-selected');
            op.running.rowSelected = null;
        },
        // 排序
        sortCol: function ($self,$et, op) {
            ...
            if (head.last && !head.data.sort && op.isPage) {
                ...
                if (op.running.sorthead) {
                    op.running.sorthead.$cell.find('.appgrid-heed-sort').hide();
                    if (head == op.running.sorthead) {

                        var $i = head.$cell.find('.appgrid-heed-sort .active');
                        $i.removeClass('active');
                        if ($i.hasClass('fa-caret-up')) {
                            head.$cell.find('.appgrid-heed-sort .fa-caret-down').addClass('active');
                            isup = false;
                        }
                        else {
                            head.$cell.find('.appgrid-heed-sort .fa-caret-up').addClass('active');
                            isup = true;
                        }
                        head.$cell.find('.appgrid-heed-sort').show();
                    }
                    else {
                        head.$cell.find('.appgrid-heed-sort .active').removeClass('active');
                        head.$cell.find('.appgrid-heed-sort .fa-caret-up').addClass('active');
                        head.$cell.find('.appgrid-heed-sort').show();
                        isup = true;
                    }
        ...
        clickRow: function ($self, $et, op) {
            ...
            if (!rownum) {
                $cell = $et.parents('.appgrid-data-cell');
                rownum = $cell.attr('rownum');
            }
            var rowIndex = parseInt(rownum.replace(op.id + '_', ''));
            // 获取当前行数据
            var rowItem = op.running.rowdata[rowIndex];
            var rowData = rowItem['appgridRowData'];
            ...
            $.each(op.running.headData, function (i, col) {
                if (col.data.name == colname) {
                    if (col.data.edit && col.data.edit.type != 'layer') {
                        ...
                        var $edit = col.$edit.find('#appgrid_edit_' + op.id + '_' + col.data.name);
                        $edit.unbind('change');
                        ...
                        switch (col.data.edit.type){
                            case 'input':          // 输入框 文本,数字,密码
                                ...
                                break;
                            case 'select':         // 下拉框选择
                                $edit.appselectSet(rowData[colname] || '');
                                $edit.on('change', function () {
                                    var $this = $(this);
                                    var value = $this.appselectGet();
                                    var item = $this.appselectGetEx();
                ...
                if (op.isMultiselect) {
                    ...
                    if ((_cbValue + '') == '1') {
                        ...
                        if (!op.running.checkAlling) {
                            ...
                            if (_flag) {
                                $self.find('#appgrid_all_cb_' + op.id).attr('src', imageurl + cb[2]);
                            }
                            else {
                                $self.find('#appgrid_all_cb_' + op.id).attr('src', imageurl + cb[0]);
                            }
                        ...
                        if (!op.running.checkAlling) {
                            ...
                            if (_flag) {
                                ...
                                $self.find('#appgrid_all_cb_' + op.id).attr('src', imageurl + cb[1]);
                            }
                        }
                    }
                }
                else {
                    $self.find('#appgrid_left_' + op.id + ' .appgrid-selected,#appgrid_right_' + op.id + ' .appgrid-selected').removeClass('appgrid-selected');
                    $self.find('[rownum="' + rownum + '"]').addClass('appgrid-selected');
        ...
        // 选中所有行
        checkAlappows: function ($self, $et, op) {
            // 判断当前是否是选中还是不选中
        ...
        expandSub: function ($self, $et, op) {
            ...
            // 
            var $body = $self.find('#appgrid_body_' + op.id);
            ...
            if (rowItem['jfsubGrid'].value) {
                ...
                var subContentId = app.newGuid();
                var $sub = $('<div class="appgrid-sub" id="' + subid + '" ><div id="' + subContentId +'" ></div></div>').css({ 'left': 0, 'top': rowItem['jfsubGrid'].top + op.rowHeight, 'height': op.subGridHeight, 'width': width });
                // 调整单元格位置
                for (var i = rowIndex + 1, l = op.running.rowdata.length; i < l; i++) {
                    var item = op.running.rowdata[i];
                    $.each(item, function (_index, _item) {
                ...
                op.subGridExpanded && op.subGridExpanded(subContentId, rowItem['appgridRowData']);
            }
        },
        // 获取选中行
        ...
        turnPage: function (e) {
            var $this = $(this);
            var $self = $('#' + $this.attr('id').replace('appgrid_page_bar_num_', ''));
            var op = $self[0].dfop;
        ...
        turnPage2: function (e) {
            ...
            var op = e.data.op;
            var name = $this.text();
            var $pagenum = $('#appgrid_page_bar_num_' + op.id);
            ...
            switch (name) {
                ...
                case '尾页':
                    ...
                    break;
                case '跳转':
                    var text = $this.parents('#appgrid_page_bar_nums_' + op.id).find('input').val();
                    if (!!text) {
                        var p = parseInt(text);
                        if (String(p) != 'NaN') {
                            if (p < 1) {
        ...
        print: function ($self) {
            ...
            $.each(dfop.rowdatas, function (index, item) {
                ...
                $self.find('[rownum="' + dfop.id + '_' + index + '"]').each(function () {
                    var $cell = $(this);
                    var colname = $cell.attr('colname');
                    if (!!colname && colname != 'appgrid_num' && colname != 'appgrid_multiselect' && colname != 'appgrid_subGrid') {
    ...
    $.fn.appgrid = function (op) {    	
        ...
        _appgrid.init($self, dfop);

        dfop = null;
        return $self;
    };

    $.fn.appGridSet = function (name, data) {  
        var $self = $(this);        
        var dfop = $self[0].dfop;
        if (!dfop) {
            return null;
        ...
        switch (name) {
            ...
            case 'refreshdata':
                ...
                _appgrid.dataRender($self, dfop, dfop.rowdatas);
                break;
            case 'refreshdatas'://将表格置空
                if (data) {                	
                    dfop.rowdatas = [];
                }              
                _appgrid.dataRender($self, dfop, dfop.rowdatas);
            ...
            case 'addRow':
                ...
                _appgrid.rowRender($self, dfop, data || {}, 0);
                $self.find('#appgrid_nodata_img_' + dfop.id).hide();
                break;
            case 'removeRow':
                $.each(dfop.rowdatas, function (_index, _item) {
                    if (dfop.mainId && data) {
                        ...
                        if (_item == dfop.running.rowSelected.appgridRowData) {
                            dfop.rowdatas.splice(_index, 1);
                            return false;
                        }
                    }
                ...
                var _$self = $self;
                setTimeout(function () {
                    _appgrid.dataRender(_$self, _$self[0].dfop, _$self[0].dfop.rowdatas, 0);
                    _$self = null;
                }, 100);
                break;
            case 'nocheck':
                if (dfop.isMultiselect) {
                    $.each(dfop.running.rowdata, function (_index, _item) {
                        if (_item['appgridRowData'][dfop.mainId] == data) {
                            if (_item['jfcheck'].value == 1) {
                                _item['jfcheck'].$cell.trigger('click');
                            }
                            return false;
            ...
            case 'updateRow':
                if (dfop.running.rowSelected) {
                    _appgrid.updateRow(dfop.running.rowSelected, dfop);
                }
                else if (dfop.running.rowdata[data]){
                    _appgrid.updateRow(dfop.running.rowdata[data], dfop);
                }
                break;
            case 'moveUp':
                return _appgrid.moveUp($self, dfop, data);
                break;
            case 'moveDown':
                return _appgrid.moveDown($self, dfop, data);
                break;
        }
        dfop = null;
        $self = null;
    ...
    $.fn.appGridGet = function (name) {
        ...
        switch (name) {
            case 'rowdata'://获取选中行数据
                if (op.isMultiselect) {
                    return _appgrid.getCheckRow($self, op);
                }
                else {
                    if (op.running.rowSelected) {
                        return op.running.rowSelected['appgridRowData'];
                    }
                    else {
                        return {};
                    }
            ...
            case 'showData':
                ...
                $.each(op.running.rowdata, function (_index, _item) {
                    ...
                    $.each(_item, function (_jindex, _jitem) {
                        if (_jindex != 'appgridRowData' && _jindex != 'jfnum' && _jindex != 'jfcheck' && _jindex != 'jfsubGrid') {
    ...
    $.fn.appGridValue = function (name) {
        ...
        if (op.isMultiselect) {
            var res = [];
            var list = _appgrid.getCheckRow($self, op);
            ...
            if (op.running.rowSelected) {
                return op.running.rowSelected['appgridRowData'][name];
            }
            else {
                return '';
            }
    ...
    // 打印处理
    $.fn.appgridPrint = function () {
        var $self = $(this);
        return _appgrid.print($self);
    }
