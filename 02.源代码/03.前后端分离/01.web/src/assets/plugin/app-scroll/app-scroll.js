...
(function ($, app, window) {
    ...
    var methods = {
        init: function ($this, callback) {
            var id = $this.attr('id');
            if (!id) {
                id = 'app_' + app. BASE_UTILS.newGuid();
                $this.attr('id', id);
            }

            $this.addClass('app-scroll-wrap');
            // 加载内容
            var $content = $this.children();

            var $scroll = $('<div class="app-scroll-box" id="' + id + '_box" ></div>');
            $this.append($scroll);
            $scroll.append($content);

            // 加载y滚动条
            var $vertical = $('<div class="app-scroll-vertical"   ><div class="app-scroll-vertical-block" id="' + id + '_vertical"></div></div>')
            $this.append($vertical);

            // 加载x滚动条
            var $horizontal = $('<div class="app-scroll-horizontal" ><div class="app-scroll-horizontal-block" id="' + id + '_horizontal"></div></div>')
        ...
        bindEvent: function ($this, $scroll, $vertical, $horizontal) { // 绑定监听事件
            ...
            // 监听鼠标移动
            $vertical.find('.app-scroll-vertical-block').on('mousedown', function (e) {
                $move.show();
                var $this = $(this).parent().parent();
                var op = $this[0].op;
                op.isYMousedown = true;
                op.yMousedown = e.pageY;
                $this.addClass('app-scroll-active');
                $this = null;
            });
            $horizontal.find('.app-scroll-horizontal-block').on('mousedown', function (e) {
                $move.show();
                var $this = $(this).parent().parent();
                var op = $this[0].op;
            ...
            top.$(document).on('mousemove', { $obj: $this }, function (e) {
                ...
                $move.hide();
                e.data.$obj.removeClass('app-scroll-active');
            });
        },
    ...
    $.fn.appscroll = function (callback) {
        $(this).each(function () {
            var $this = $(this);
    ...
    $.fn.appscrollSet = function (name, data) {
        switch (name) {
            case 'moveRight':
                var $this = $(this);
                setTimeout(function () {
