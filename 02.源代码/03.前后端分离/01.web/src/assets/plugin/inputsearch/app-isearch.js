 ...
 * 创建人：力软-前端开发组
 * 日 期：2017.03.22
 * 描 述：appISearch 输入搜索框，支持input输入框，数据异步加载，本地查询
 */
(function ($, app) {
    "use strict";
    $.appisearch = {
        init: function ($self) {
            var dfop = $self[0]._appisearch.dfop;
    ...
    $.fn.appisearch = function (op) {
        ...
        $self[0]._appisearch = {dfop:dfop};

        $.appisearch.init($self);
        //加载数据
        if (!!dfop.url) {
            app.HTTP_REQUEST_UTILS.httpAsync(dfop.method, dfop.url, dfop.param, function (data) {
                $self[0]._appisearch.dfop.data = data || [];
                dfop.isload = true;
                if (dfop._isSearchneed) {
                    $.appisearch.triggerSearch($self);// 触发查询函数
                }
