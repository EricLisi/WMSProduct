...
(function ($, app) {
    ...
    $.extend(app, {
        ...
        layerConfirm: function (_msg, callback) {
            top.app.language.get(_msg, function (msg) {
                top.layer.confirm(msg, {
                    ...
                    success: function (layero, index) {
                        layero.find('.layui-layer-btn a').each(function () {
                            var $this = $(this);
                            var _text = $this.text();
                            top.app.language.get(_text, function (text) {
                                $this.text(text);
                            });
        ...
        layerForm: function (op) {
            ...
            var r = top.layer.open({
                ...
                btn: dfop.btn,
                content: op.url,
                skin: dfop.btn == null ? 'app-layer-nobtn' : 'app-layer',
                success: function (layero, index) {
                    top['layer_' + dfop.id] = app.iframe($(layero).find('iframe').attr('id'), top.frames);
                    layero[0].app_layerid = 'layer_' + dfop.id;
                    //如果底部有按钮添加-确认并关闭窗口勾选按钮
                    if (!!dfop.btn && layero.find('.app-layer-btn-cb').length == 0) {
                        top.app.language.get('确认并关闭窗口', function (text) {
                            layero.find('.layui-layer-btn').append('<div class="checkbox app-layer-btn-cb" myIframeId="layer_' + dfop.id + '" ><label><input checked="checked" type="checkbox" >' + text + '</label></div>');
                        });
                        layero.find('.layui-layer-btn a').each(function () {
                            var $this = $(this);
                            var _text = $this.text();
        ...
        layerClose: function (name, index) {
            ...
            var $IsClose = layero.find('.layui-layer-btn').find(".app-layer-btn-cb input");
            ...
            if (IsClose) {
                ...
                top[layero[0].app_layerid].location.reload();
            }
