...
namespace KGMFramework.WebApp.DALSQL
{
    ...
    public class SO_Head : BaseDALSQL<SO_HeadInfo>, ISO_Head
    {
        ...
        {
            string sql = "select F_Status from SO_Head where F_Id='" + Id + "'";
            if (this.GetDataTableBySql(sql).Rows[0][0].ToString() == "1")
            {
                string updatesql = string.Format("update SO_Head set F_Status=0,F_VeriDate='{0}',F_AuditingUser='{1}' where F_Id='{2}'", date, user, Id);
                Hashtable hash = new Hashtable();
        ...
        public void DeleteAll(string keyValue, bool bLogicDelete = false)
        {
            using (DbTransactionScope<SO_HeadInfo> dbtran = base.CreateTransactionScope())
            {
                try
                {
                    ...
                    SO_Head.Instance.Delete(keyValue, dbtran.Transaction);
                    SearchCondition condition = new SearchCondition();
                    condition.AddCondition("F_Id", keyValue, SqlOperator.Equal);
                    SO_Head.Instance.DeleteByCondition(condition.BuildConditionSql().Replace("Where (1=1)  AND ", string.Empty), dbtran.Transaction);
                    dbtran.Commit();
        ...
        public DataTable GetPrint(string sourceData)
        {
            ...
            List<SO_HeadInfo> hinfo = BLLFactory<SO_Head>.Instance.Find(condition.BuildConditionSql().Replace("Where (1=1)  AND ", string.Empty));

            condition = new SearchCondition();
            condition.AddCondition("F_HId", sourceData, SqlOperator.Equal);
            List<SO_BodyInfo> binfo = SO_Body.Instance.Find(condition.BuildConditionSql().Replace("Where (1=1)  AND ", string.Empty));
